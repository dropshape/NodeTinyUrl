...
function Route(logger, config, webServer, urlShortener) {
    ...
    function createShortUrl(req, res) {
        var id = req.params.id;
        if (id) {
            urlShortener.shorten(id, {userId: req.user}, function (err, shortUrl) {
                if (err) {
                    logger.error(err);
                    return res.apiMessage(Status.BAD_REQUEST, 'error creating short url', id);
                }
                var url = {
                    protocol: protocol,
                    host: host,
            ...
            res.apiMessage(Status.BAD_REQUEST, 'error creating short url', id);
        }
    }
}
